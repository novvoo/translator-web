...
### 文档格式支持
- ✅ **EPUB 电子书** - 完整支持 EPUB 格式，输出双语对照的 EPUB 文件
- ✅ **PDF 文档** - 智能提取 PDF 文本内容，输出双语对照的 HTML 文件
...
### AI 翻译引擎
- ✅ **统一翻译接口** - EPUB 和 PDF 使用相同的翻译流程
- ✅ **智能缓存系统** - 自动缓存翻译结果，避免重复翻译
- ✅ **强制重译选项** - 支持忽略缓存强制重新翻译

## 功能特性

- 📚 **EPUB 翻译** - 上传 EPUB 文件，输出双语对照的 EPUB 电子书
- 📄 **PDF 翻译** - 上传 PDF 文件，输出双语对照的 HTML 文件
- 🔒 **多用户会话隔离** - 每个用户的数据完全独立，支持公共部署
- 🤖 **支持多种 AI 提供商**：
...
| 格式 | 输入 | 输出 | 说明 |
|------|------|------|------|
| **EPUB** | .epub | .epub | 双语对照的电子书，保持原有格式和结构 |
| **PDF** | .pdf | .html | 双语对照的 HTML 文件，包含原文和译文 |
...
2. **配置翻译参数**：
   - **选择 AI 提供商**：OpenAI、Claude、Gemini、DeepSeek、Ollama、NLTranslator、LibreTranslate 或自定义
   - 选择目标语言
   - 输入 API Key（本地模型和部分服务无需）
...
5. **下载结果**：翻译完成后点击"下载翻译文件"
   - EPUB 文件：下载双语对照的 .epub 文件
   - PDF 文件：下载双语对照的 .html 文件
...
Model: gpt-4 (高质量) 或 gpt-3.5-turbo (快速)
API Key: sk-...
Temperature: 0.3 (平衡质量和一致性)
```
...
| **自定义API** | 灵活 | 取决于服务 | 取决于服务 | 取决于服务 | 支持任何 OpenAI 兼容接口 |

### 使用建议

- **高质量翻译**：推荐 OpenAI GPT-4 或 Claude-3.5
- **性价比优先**：推荐 DeepSeek 或 Gemini
- **完全免费**：推荐 Ollama（需要本地部署）或 LibreTranslate
- **隐私安全**：推荐 Ollama 或 NLTranslator（macOS）
- **快速翻译**：推荐 NLTranslator 或 DeepSeek
...
translator-web/
├── backend/                     # Go 后端
│   ├── main.go                 # 主程序入口，包含会话管理
│   ├── handlers/               # API 处理器
│   │   └── translate.go        # 翻译相关 API，支持多用户隔离
│   ├── middleware/             # 中间件
│   │   └── session.go          # 会话管理中间件
│   ├── models/                 # 数据模型
│   │   └── task.go             # 任务模型
...
**参数**:
- `file`: 文档文件（.epub 或 .pdf）
- `targetLanguage`: 目标语言
- `llmConfig`: LLM 配置（JSON 字符串）
  - `provider`: 提供商类型（openai/claude/gemini/deepseek/ollama/nltranslator/libretranslate/custom）
  - `apiKey`: API Key（本地模型和部分服务可选）
  - `apiUrl`: API URL
  - `model`: 模型名称
...
- `forceRetranslate`: 强制重新翻译（可选，true/false）
...
下载翻译后的文件
- EPUB 文件：返回双语对照的 .epub 文件
- PDF 文件：返回双语对照的 .html 文件
...
- 建议使用 GPT-4 或 Claude-3.5 以获得更好的翻译质量
- API Key 仅在内存中使用，不会被存储
- 每个用户的数据完全隔离，支持多用户同时使用
- PDF 翻译结果为 HTML 格式，便于查看和打印
- 支持翻译缓存，相同内容不会重复翻译
