...
func (dt *DocumentTranslator) TranslateDocument(inputPath, outputPath, targetLanguage, userPrompt string, progressCallback func(float64)) (string, error) {
	...
	case DocumentTypePDF:
		...
		actualOutputPath, err := dt.savePDFTranslation(pdfDoc, outputPath, textBlocks, translations)
		if err != nil {
...
func (dt *DocumentTranslator) savePDFTranslation(pdfDoc *PDFDocument, outputPath string, originalBlocks []string, translations map[string]string) (string, error) {
	// 构建翻译后的文本块
	...
	default:
		...
		return htmlPath, pdfDoc.SaveBilingualHTML(htmlPath, originalBlocks, translatedBlocks)
	}
}
